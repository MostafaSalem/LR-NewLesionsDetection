...
Patient_Nums={'717','818'}; % you can add any number of patients as you want

load('Path to code folder/LogisticRegressionModel.mat')
...
for patient=1:size(Patient_Nums,2)
    ...
    atlas = load_nifti([DataPath,Patient_Nums{patient},'/12M/atlas/atlas_similarity']);
    ...
    % #############################################################################################
    display('Testing');
    display(['Testing with Patient No. ',Patient_Nums{patient}]);

    load('Path to code folder/LogisticRegressionModel.m','LogisticRegressionModel');
    [label,Probability]=LogisticRegressionModel.predictFcn(TestingTable);
    
    mkdir([DataPath,Patient_Nums{patient},'/12M/LR'])
    mkdir([DataPath,Patient_Nums{patient},'/12M/LR/A'])
    mkdir([DataPath,Patient_Nums{patient},'/12M/LR/B'])
    mkdir([DataPath,Patient_Nums{patient},'/12M/LR/C'])
    mkdir([DataPath,Patient_Nums{patient},'/12M/LR/D'])
    mkdir([DataPath,Patient_Nums{patient},'/12M/LR/E'])
    
    %The probability Image (The original and smoothing ones)
    probaA=zeros(size(atlas.img),'single');
    probaA(voxelSelectionImage)=Probability(:,1);
    atlas.img = probaA;
    save_nifti(atlas, [DataPath,Patient_Nums{patient},'/12M/LR/probability_0.0'],'u');
    probaB = imgaussfilt3(probaA,0.25);
    atlas.img = probaB;
    save_nifti(atlas, [DataPath,Patient_Nums{patient},'/12M/LR/probability_0.25'],'u');
    probaC = imgaussfilt3(probaA,0.5);
    atlas.img = probaC;
    save_nifti(atlas, [DataPath,Patient_Nums{patient},'/12M/LR/probability_0.5'],'u');
    probaD = imgaussfilt3(probaA,0.75);
    atlas.img = probaD;
    save_nifti(atlas, [DataPath,Patient_Nums{patient},'/12M/LR/probability_0.75'],'u');
    probaE = imgaussfilt3(probaA,1.0);
    atlas.img = probaE;
    save_nifti(atlas, [DataPath,Patient_Nums{patient},'/12M/LR/probability_1.0'],'u');
    
    % Create segmentations by thresholding the probability maps with [0,0.1,0.2,....1]
    for i=0.0:0.1:1
        %WithoutSmoothing
        ProbabilityThreshold = (probaA(voxelSelectionImage)>=i);
        newGTImage=uint8(zeros(size(gt.img)));
        newGTImage(voxelSelectionImage)=ProbabilityThreshold;
        gt.img = newGTImage;
        save_nifti(gt, [DataPath,Patient_Nums{patient},'/12M/LR/A/Seg_',num2str(i)],'u');
        %WithSmoothing sigma 0.25
        ProbabilityThreshold = (probaB(voxelSelectionImage)>=i);
        newGTImage=uint8(zeros(size(gt.img)));
        newGTImage(voxelSelectionImage)=ProbabilityThreshold;
        gt.img = newGTImage;
        save_nifti(gt, [DataPath,Patient_Nums{patient},'/12M/LR/B/Seg_',num2str(i)],'u');
        %WithSmoothing sigma 0.5
        ProbabilityThreshold = (probaC(voxelSelectionImage)>=i);
        newGTImage=uint8(zeros(size(gt.img)));
        newGTImage(voxelSelectionImage)=ProbabilityThreshold;
        gt.img = newGTImage;
        save_nifti(gt, [DataPath,Patient_Nums{patient},'/12M/LR/C/Seg_',num2str(i)],'u');
        %WithSmoothing sigma 0.75
        ProbabilityThreshold = (probaD(voxelSelectionImage)>=i);
        newGTImage=uint8(zeros(size(gt.img)));
        newGTImage(voxelSelectionImage)=ProbabilityThreshold;
        gt.img = newGTImage;
        save_nifti(gt, [DataPath,Patient_Nums{patient},'/12M/LR/D/Seg_',num2str(i)],'u');
        %WithSmoothing sigma 1.0
        newGTImage=uint8(zeros(size(gt.img)));
        newGTImage(voxelSelectionImage)=ProbabilityThreshold;
        gt.img = newGTImage;
        save_nifti(gt, [DataPath,Patient_Nums{patient},'/12M/LR/E/Seg_',num2str(i)],'u');
